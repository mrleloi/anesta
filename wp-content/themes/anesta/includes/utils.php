...
if ( ! function_exists( 'anesta_array_insert_after' ) ) {
	...
	function anesta_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		$haystack = is_int( $needle )
						...
						: anesta_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );
...
if ( ! function_exists( 'anesta_getimagesize' ) ) {
	 ...
	 */
	function anesta_getimagesize( $url, $echo = false ) {
		$img_size = false;
		$img_path = anesta_is_url( $url ) ? anesta_url_to_local_path( $url ) : $url;
...
if ( ! function_exists( 'anesta_get_embed_video' ) ) {
	...
	function anesta_get_embed_video( $video, $use_wp_embed = false ) {
		...
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			...
			$video = anesta_add_to_url( $video, anesta_get_embed_video_parameters() );
			$tag   = anesta_get_embed_video_tag_name();
					...
					if ( ! empty( $matches[2] ) ) {
						...
						$matches[2] = anesta_add_to_url(
							...
							array_merge(
								...
								$muted
									? array_merge(
										anesta_get_embed_video_parameters(),
										array(
											'muted' => 1,
											'controls' => 0,
											'background' => 1,
	...
	function anesta_fgc( $file ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fgc' ) ) {
				...
				global $wp_filesystem;
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					$file = substr( $file, 0, 2 ) == '//' ? anesta_add_protocol( $file ) : str_replace( ABSPATH, $wp_filesystem->abspath(), $file );
...
if ( ! function_exists( 'anesta_unzip_file' ) ) {
	/**
	 ...
	 */
	function anesta_unzip_file( $zip, $dest ) {
...
if ( ! function_exists( 'anesta_add_to_url' ) ) {
	...
	function anesta_add_to_url( $url, $prm ) {
		if ( is_array( $prm ) && count( $prm ) > 0 ) {
			...
			$params = anesta_array_merge( $params, $prm );
			$url = $parts[0];
			$separator = '?';
...
if ( ! function_exists( 'anesta_url_to_local_path' ) ) {
	/**
	 ...
	 */
	function anesta_url_to_local_path( $url ) {
		$path = '';
