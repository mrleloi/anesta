...
if ( function_exists( 'anesta_elementor_is_preview' ) && anesta_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			anesta_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	anesta_new_main_query(
		array(
			'post_type'      => anesta_get_theme_option( 'post_type' ),
			'category'       => anesta_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'anesta_filter_get_template_part', anesta_blog_archive_get_template() ) );
}
