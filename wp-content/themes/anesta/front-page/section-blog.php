<div class="front_page_section front_page_section_blog<?php
	$anesta_scheme = anesta_get_theme_option( 'front_page_blog_scheme' );
	if ( ! empty( $anesta_scheme ) && ! anesta_is_inherit( $anesta_scheme ) ) {
		echo ' scheme_' . esc_attr( $anesta_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( anesta_get_theme_option( 'front_page_blog_paddings' ) );
	if ( anesta_get_theme_option( 'front_page_blog_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$anesta_css      = '';
		$anesta_bg_image = anesta_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$anesta_anchor_icon = anesta_get_theme_option( 'front_page_blog_anchor_icon' );
	$anesta_anchor_text = anesta_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( anesta_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$anesta_css      = '';
			$anesta_bg_mask  = anesta_get_theme_option( 'front_page_blog_bg_mask' );
			$anesta_bg_color_type = anesta_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $anesta_bg_color_type ) {
				$anesta_bg_color = anesta_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$anesta_caption = anesta_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $anesta_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $anesta_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $anesta_caption, 'anesta_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$anesta_description = anesta_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $anesta_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $anesta_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $anesta_description ), 'anesta_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_blog_output">
				...
				if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
					...
					if ( ! anesta_exists_trx_addons() ) {
						...
						anesta_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
					}
				}
				?>
			</div>
