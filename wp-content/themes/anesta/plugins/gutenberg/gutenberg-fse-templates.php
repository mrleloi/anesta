...
if ( ! function_exists( 'anesta_gutenberg_fse_replace_no_posts_content' ) ) {
	add_filter( 'search_template',  'anesta_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	add_filter( 'index_template',   'anesta_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	add_filter( 'archive_template', 'anesta_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	/**
	 * Substitute a content of the global variable $_wp_current_template_content
	 * with a content of the template 'parts/none-archive.html' or 'templates/none-archive.html'.
	 * 
	 * Hooks: add_filter( 'search_template',  'anesta_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	 * 
	 *        add_filter( 'index_template',   'anesta_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	 * 
	 *        add_filter( 'archive_template', 'anesta_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	 ...
	 * @param string[] $templates A list of template candidates, in descending order of priority.
	 */
	function anesta_gutenberg_fse_replace_no_posts_content( $template, $type = '', $templates = array() ) {
		if ( substr( $template, -19 ) == 'template-canvas.php' ) {
			if ( ! have_posts() ) {
				...
				$no_posts_file = anesta_get_file_dir( ( $part_exists ? 'parts/' : 'templates/' ) . "{$part_name}.html" );
				if ( ! empty( $no_posts_file ) ) {
					// Check if exists a saved template with a same name
					$posts = get_posts( array(
						'name'           => $part_name,
						'posts_per_page' => 1,
						'post_type'      => defined( 'ANESTA_FSE_TEMPLATE_PART_PT' ) ? ANESTA_FSE_TEMPLATE_PART_PT : 'wp_template_part',
					) );
					// Get a template content from the database or from the file (if a saved version is not found)
					$content = ! empty( $posts[0]->post_content ) ? $posts[0]->post_content : anesta_fgc( $no_posts_file );
...
if ( ! function_exists( 'anesta_gutenberg_fse_show_template_as_layout' ) ) {
	add_filter( 'anesta_filter_custom_layout_shown', 'anesta_gutenberg_fse_show_template_as_layout', 10, 3 );
	/**
	 * Show an FSE template (header/footer/sidebar) inside a non-FSE templates.
	 * 
	 * Hook: add_filter( 'anesta_filter_custom_layout_shown', 'anesta_gutenberg_fse_show_template_as_layout', 10, 3 );
	 * 
	 ...
	 */
	function anesta_gutenberg_fse_show_template_as_layout( $shown, $layout_id = '', $post_id = 0 ) {
		// If a layout_id is a string with a template part name
		if ( ! $shown && ! empty( $layout_id ) && is_string( $layout_id) && (int)$layout_id === 0 && anesta_gutenberg_is_fse_theme() ) {
			// And file with template-name.html is present in the theme subfolder '/parts'
			$template_path = anesta_get_file_dir( "/parts/{$layout_id}.html" );
			if ( ! empty( $template_path ) ) {
				$template_content = anesta_fgc( $template_path );
				if ( ! empty( $template_content ) ) {
					anesta_show_layout( anesta_filter_post_content( $template_content ) );
					$shown = true;
				}
			}
		}
...
if ( ! function_exists( 'anesta_gutenberg_fse_modify_template_replace_macros' ) ) {
//	add_filter( 'anesta_filter_wp_current_template_content', 'anesta_gutenberg_fse_modify_template_replace_macros', 100, 4 );
	/**
	 * Replace a macros {{Y}} and {Y} in the content of the current template to the current year.
	 * 
	 * Hooks: add_filter( 'anesta_filter_wp_current_template_content', 'anesta_gutenberg_fse_modify_template_replace_macros', 10, 4 );
	 *
	 ...
	 */
	function anesta_gutenberg_fse_modify_template_replace_macros( $content, $template, $type, $templates ) {
...
// Sidebar
//----------------------------------------------
if ( ! function_exists( 'anesta_gutenberg_fse_add_sidebar_to_default_areas' ) ) {
	add_filter( 'default_wp_template_part_areas', 'anesta_gutenberg_fse_add_sidebar_to_default_areas' );
	/**
	 * Filters the list of allowed template part area values and add 'sidebar' to this list.
	 * 
	 * Hook: add_filter( 'default_wp_template_part_areas', 'anesta_gutenberg_fse_add_sidebar_to_default_areas' );
	 *
	 ...
	 */
	function anesta_gutenberg_fse_add_sidebar_to_default_areas( $default_area_definitions ) {
		$default_area_definitions[] = array(
			'area'        => ANESTA_FSE_TEMPLATE_PART_AREA_SIDEBAR,
			'label'       => esc_html__( 'Sidebar', 'anesta' ),
			'description' => esc_html__( 'The Sidebar template defines a page area that typically contains a widgets.', 'anesta' ),
			'icon'        => 'sidebar',
			'area_tag'    => 'div',
		);
	...
	function anesta_gutenberg_fse_modify_template_replace_sidebar( $content, $template, $type, $templates ) {
		...
		if ( anesta_sidebar_present() ) {
			...
			if ( anesta_exists_trx_addons() ) {
				...
				$sidebar_start = apply_filters( 'anesta_filter_wp_block_with_sidebar_mask_start',
												'(<!-- wp:group[\s]*{[^}]*"className":[\s]*"[^"]*sidebar[^"]*"[^>]*-->[\s]*'
												. '<div[^>]*class="[^"]*sidebar[^"]*"[^>]*>)'
												);
				$sidebar_end = apply_filters( 'anesta_filter_wp_block_with_sidebar_mask_end',
												'(</div>[\s]*'
												. '<!-- /wp:group -->)'
												);
				...
				if ( 'default' == $sidebar_type ) {
					...
					if ( is_active_sidebar( $sidebar_name ) ) {
						...
						if ( preg_match( $sidebar_mask, $content ) ) {
							...
							$need_wrapper = apply_filters( 'anesta_filter_wp_block_with_sidebar_wrapper_need', false );
							$content = preg_replace( '#<!-- wp:template-part[\s]*{[^}]*"slug":[\s]*"sidebar[^"]*"[^>]*/-->#U',
														( $need_wrapper
															? apply_filters(
																	'anesta_filter_wp_block_with_sidebar_wrapper_start',
														...
														. ( $need_wrapper
															? apply_filters(
																	'anesta_filter_wp_block_with_sidebar_wrapper_end',
																	'</div>'
																	. "\n"
																	. '<!-- /wp:group -->'
																)
				...
				// A custom sidebar (built with FSE or with any other builder)
					...
					// Custom sidebar's layout
						...
						// Custom sidebar
						do_action( 'anesta_action_show_layout', $sidebar_id );
