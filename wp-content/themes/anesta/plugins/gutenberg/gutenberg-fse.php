...
if ( ! defined( 'ANESTA_FSE_TEMPLATE_PART_AREA_HEADER' ) )  define( 'ANESTA_FSE_TEMPLATE_PART_AREA_HEADER',  defined( 'WP_TEMPLATE_PART_AREA_HEADER' )  ? WP_TEMPLATE_PART_AREA_HEADER  : 'header' );
if ( ! defined( 'ANESTA_FSE_TEMPLATE_PART_AREA_FOOTER' ) )  define( 'ANESTA_FSE_TEMPLATE_PART_AREA_FOOTER',  defined( 'WP_TEMPLATE_PART_AREA_FOOTER' )  ? WP_TEMPLATE_PART_AREA_FOOTER  : 'footer' );
if ( ! defined( 'ANESTA_FSE_TEMPLATE_PART_AREA_SIDEBAR' ) ) define( 'ANESTA_FSE_TEMPLATE_PART_AREA_SIDEBAR', defined( 'WP_TEMPLATE_PART_AREA_SIDEBAR' ) ? WP_TEMPLATE_PART_AREA_SIDEBAR : 'sidebar' );
if ( ! defined( 'ANESTA_FSE_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ) define( 'ANESTA_FSE_TEMPLATE_PART_AREA_UNCATEGORIZED', defined( 'WP_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ? WP_TEMPLATE_PART_AREA_UNCATEGORIZED : 'uncategorized' );
...
if ( ! function_exists( 'anesta_gutenberg_fse_skin_updated' ) ) {
	add_action( 'anesta_action_theme_updated', 'anesta_gutenberg_fse_skin_updated' );
	add_action( 'anesta_action_skin_updated', 'anesta_gutenberg_fse_skin_updated', 10, 1 );
	/**
	 * Copy FSE-required folders from an updated skin's subfolder '_fse' to the theme's root folder.
	 * 
	 * Hooks: add_action( 'anesta_action_skin_updated', 'anesta_gutenberg_fse_skin_updated', 10, 1 );
	 */
	function anesta_gutenberg_fse_skin_updated( $skin = '' ) {
		// Get an active skin name if empty
		if ( current_action() == 'anesta_action_theme_updated' && empty( $skin ) ) {
			$skin = anesta_skins_get_active_skin_name();
		}
		if ( anesta_skins_get_current_skin_name() == $skin ) {
			anesta_gutenberg_fse_copy_from_skin( $skin );
			anesta_gutenberg_fse_update_theme_json( $skin );
		}
	}
}
...
if ( ! function_exists( 'anesta_gutenberg_fse_update_theme_json' ) ) {
	...
	function anesta_gutenberg_fse_update_theme_json( $new_skin = '', $old_skin = '' ) {
		// Get an active skin name if empty
		if ( in_array( current_action(), array( 'anesta_action_save_options', 'trx_addons_action_save_options' ) ) && empty( $new_skin ) ) {
			$new_skin = anesta_skins_get_active_skin_name();
		}
		// If the current skin supports FSE
		if ( ! empty( $new_skin ) && is_dir( ANESTA_THEME_DIR . anesta_skins_get_current_skin_dir( $new_skin ) . '_fse' ) ) {
		...
		$data = array(
					...
					"templateParts" => array(
						array(
							...
							"area"  => ANESTA_FSE_TEMPLATE_PART_AREA_HEADER,
						),
						array(
							"name"  => "sidebar",
						...
						array(
							"name"  => "none-archive",
							"title" => esc_html__( "No posts found", 'anesta' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "none-search",
							"title" => esc_html__( "Nothing matched search", 'anesta' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
