...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			anesta_create_observer( 'anesta-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				anesta_gutenberg_editor_init();
			} );
		} else {
		 	anesta_gutenberg_editor_init();
		}
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'anesta_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'anesta_inited' ) ) {
	...
	// Init on page load
	function anesta_gutenberg_editor_init() {
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + ANESTA_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + ANESTA_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in ANESTA_STORAGE['override_classes'] ) {
				$obj.addClass( ANESTA_STORAGE['override_classes'][i].replace( '%s', ANESTA_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( anesta_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && ANESTA_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'anesta_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			anesta_remove_observer( 'anesta-check-editor-styles-wrapper' );
			anesta_create_observer( 'anesta-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'anesta_inited' );
	}
} );
