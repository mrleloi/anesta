...
if ( ! function_exists( 'trx_addons_enqueue_tweenmax' ) ) {
	...
	function trx_addons_enqueue_tweenmax( $args = array() ) {
		...
		wp_enqueue_script( 'tweenmax', trx_addons_get_file_url('js/tweenmax/GSAP/3.12.2/gsap.min.js'), array(), null, true );
		if ( ! empty( $args['ScrollTo'] ) ) {
			wp_enqueue_script( 'tweenmax-plugin-scrolltoplugin', trx_addons_get_file_url('js/tweenmax/GSAP/3.12.2/ScrollToPlugin.min.js'), array(), null, true );
		}
		if ( ! empty( $args['ScrollTrigger'] ) ) {
			wp_enqueue_script( 'tweenmax-plugin-scrolltrigger', trx_addons_get_file_url('js/tweenmax/GSAP/3.12.2/ScrollTrigger.min.js'), array(), null, true );
		}
	}
}
